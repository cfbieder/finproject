...
| `/refresh-ps` | RefreshPS | Transactions | **(CR022) Titled "Refresh Feeds"** — handles both PocketSmith and bank-feed. Tabbed view (Review & Edit New / New Transactions / Modified) with inline transaction editing using shared `TransactionTable` + `TransactionEditModal` + `CategorySelector`. Two refresh buttons: **Refresh PS Data** (`POST /ingest-ps/refresh-ps`) and **Refresh Feed Data** (`POST /ingest-bank-feed/refresh`, reuses the Days window). Selection checkboxes for multi-select bulk operations. Header actions: **Suggest categories** (fills uncategorized rows from history via `POST /transactions/category-suggestions`, pending), **Accept PS** / **Accept Bank Feed** (accept all queued rows of one source), **Accept All**; bulk Category button appears when rows selected. The review table shows a **Source** column (PS / Bank feed / Quicken) so mixed-source rows are distinguishable. **Group by account** toggle clusters the review rows by account (alphabetical groups, each with a header row showing account name + count; within-group order follows the active column sort) via `TransactionTable`'s optional `groupByKey` prop. Per-row action buttons for Category, Split, Neutralize, **Transfer** (record as a cross-account transfer → `POST /transactions/:id/transfer`, opens an `AccountPicker`), and Accept. Clickable Date, Description, and Category cells open edit modals. **Date edit:** Click date cell to open modal with `<input type="date">`, saves via `PATCH /api/v2/transactions/:id`. When the date changes and the transaction currency differs from base currency, the server automatically recalculates `base_amount` using an implied FX rate derived from a nearby transaction (same currency, ±3 days, largest amount for stability). Toast shows old → new USD amount and the implied rate. **Bulk Category:** Select multiple rows via checkboxes, click "Category (N)" button to assign same category to all selected. **Split:** Per-row button opens a modal to divide the original amount across 2-5 entries with optional category selection. Uses `POST /api/v2/transactions/:id/split`. **Neutralize:** Per-row button creates an offsetting entry for brokerage security trades (cash-for-shares exchange). Both transactions categorized as "Transfer - Securities Trades" and marked accepted. Uses `POST /api/v2/transactions/:id/neutralize`. |
...
**Routing (`App.jsx` → `AppShell`):** Mobile pages live under `/m/*`. A top-level effect inside the router redirects desktop URLs → `/m/*` when on mobile, and `/m/*` → desktop URLs when not on mobile, using a `DESKTOP_TO_MOBILE` map covering the 7 mobile pages plus `/` → `/m`. Mobile and desktop layouts are mutually exclusive (no shared chrome).
