...
**Quicken import (CR019, in-progress):** `server/src/v2/scripts/quicken-import.js` is the Phase B parser CLI — reads QIFs and stages cash-account rows into `quicken_staging`. Investment types (Phase D) and promote (Phase E) extend the same module. CLI usage: `node server/src/v2/scripts/quicken-import.js parse --files <path>[:CURRENCY][,...] --batch <uuid> [--label "<text>"]`. The same `runParse()` is also exposed UI-side via `POST /api/v2/quicken-import/parse` (body `{files:[{name,currency,content}]}`) — the admin UI's **New Import** modal uploads QIF text and creates **one batch per file** (per-account rollback/verify granularity). Admin routes in `server/src/v2/routes/quickenImport.js`: `POST /parse`, `GET /batches`, `GET /batches/:id`, `POST|DELETE /batches/:id/mappings`, `GET /batches/:id/preflight`, `POST /batches/:id/promote`, `POST /batches/:id/rollback`. Post-promote correctness check: `node server/src/v2/scripts/quicken-verify.js --batch <uuid> [--expect-account <id>] [--source <name>]` (cross-source-overlap, account integrity, calibration balance invariant; within-import dupes + uncategorized as warnings). Test fixtures live under `Samples/quicken/fixtures/`. Tests at `server/src/v2/scripts/__tests__/quicken-import.test.js` (18 tests; pure-parsing tests run anywhere; DB-backed tests need dev Postgres on `localhost:5434`, skippable via `SKIP_DB_TESTS=1`).
