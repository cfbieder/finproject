...
<a id="cr040"></a>
- **CR040 — [Forecast Scenario Compare](CRs/CR040_FORECAST_SCENARIO_COMPARE.md)** — *IN-PROGRESS — P1+P2 built & e2e-verified on dev 2026-07-10 (not yet released).* `/forecast-compare` (FC step 6): baseline A vs comparison B, KPI deltas + full P&L/BS delta grids (client-side diff reconciling with Review, incl. bank running-balance seeding; base-year double-count caught and fixed via live e2e), recharts A-vs-B lines + diverging delta bars (dataviz-validated palettes, light+dark), deterministic "where they differ" commentary. 13 new unit tests (116 total green). **Open:** P3 — on-demand local-LLM narrative with follow-ups (aiReview `compareWith` extension); release/deploy; delete the "CR040 Test B" scenario from dev after browser testing.
