...
<a id="cr022"></a>
- **CR022 — [Bank Feed Parallel Import](CRs/CR022_BANK_FEED_PARALLEL_IMPORT.md)** — *IN-PROGRESS* (Phase A migration 023 + Phase B converter/staging/dedup shipped in v2.8.0, 2026-05-31). Additive second import route in fin that consumes the CR021 bank-feed `/v1/*` contract and writes into the existing `transactions` table with `source='bank-feed'`, alongside PocketSmith. Scope: migration `023_bank_feed_import.sql` (one new column `bank_feed_external_id`, partial-unique index, parallel `bankfeed_staging` table, `sync_metadata` seed) + new converter/repository/orchestrator/route files under `server/src/v2/` + Jest unit + DB-backed promote tests + `smoke-bank-feed.js`. Reuses the existing review queue (already source-agnostic) and `account_source_mappings` (already source-discriminated). **Two must-have requirements (§2.3, added 2026-05-31, now propagated through §3–§9):** **R1** per-account opt-in — new `account_source_mappings.ignored` flag; an unmapped account stages but never promotes (fail-closed, no silent ingest). **R2** cross-source dedup — at promote time a bank-feed row links onto a matching `source='pocketsmith'` row (`(account_id, ABS(amount), currency)` ±1 day) instead of inserting, keeping the canonical `id` stable; the reverse direction drops the later PS row via one guarded lookup in `refreshPsApiV2.js` (the only PS-code edit, behind `BANK_FEED_DEDUP_ENABLED`, default true); `merged_with_ps_count` is the parallel-run health signal. **Hard gate:** numbered dev walkthrough on `fin-server-dev` (with `pg_dump` rollback bracket, now also exercising the R1 ignore toggle + an R2 synthetic duplicate) before any prod push. **Non-goals:** removing PocketSmith, generalizing `pending_transactions`, modifying the existing `/refresh-ps` *UI*, investment-side schema. PS removal becomes a separate future CR after ≥1-month parallel-run observation. **Coordinating dependency:** bank-feed's `fintableSync.js` hardcodes the PKO institution onto every account — needs a CR021-side fix before Fidelity arrives (Fidelity is not yet in the bank-feed; investigation 2026-05-30 confirms still 7 PKO accounts only).
