...
<a id="cr038"></a>
- **CR038 — [Home Dashboard & Attention Surface](CRs/CR038_HOME_DASHBOARD_ATTENTION.md)** — *PLANNED (scoped 2026-07-03).* Desktop Home (currently a zero-data link farm) adopts MobileHome's live KPIs via a shared `useOverview` hook; a "needs attention" strip (unreviewed tx / stale feeds per CR035 thresholds / drift + MTM gap / KI#7 verify-USD rows) plus next-step prompts tie the weekly refresh→review→reconcile loop together. Optional P4: minimal mobile reconcile page. Open owner decisions: strip placement (Home vs also TopStrip badge); P4 in/out.
