...
<a id="cr038"></a>
- **CR038 — [Home Dashboard & Attention Surface](CRs/CR038_HOME_DASHBOARD_ATTENTION.md)** — *P1–P3 RELEASED v3.0.55 (2026-07-03; decisions settled: strip on Home only). **Open: P4** — minimal mobile reconcile page (deferred).* Desktop Home shows live KPIs (net worth + MoM delta, this-month income/expense/net) via the shared `useOverview` hook (MobileHome consumes it too — first mobile-dedup slice); `GET /util/attention-summary` + `AttentionStrip` pills (unreviewed tx / KI#7 verify-USD / stale feeds per CR035 thresholds / fed+manual drift) each linking to its clearing page; next-step prompts on Refresh Feeds and Balance Reconciliation.

<a id="cr039"></a>
- **CR039 — [Forecast Assumptions to Postgres](CRs/CR039_FORECAST_ASSUMPTIONS_TO_DB.md)** — *PLANNED (scoped 2026-07-03).* Retire the `FCAssump.json` dual source of truth (file+DB merge and sync `fs` writes in `routes/forecast.js:41-125`); migrate to a DB store with byte-identical API responses, one-time import script, debug-CSV engine writes behind `FC_DEBUG_CSV`. **Prerequisite for CR027** — do before v4 schema work.
...
<a id="cr036"></a>
- **CR036 — [Manual Statement Upload (Stale-Feed Fallback)](CRs/CR036_MANUAL_STATEMENT_UPLOAD.md)** — *P1 SHIPPED & deployed 2026-07-01 — bank-feed service (commit `91c2911`) + fin **v3.0.45**; UI declutter/polish in **v3.0.46–v3.0.47**. Live-verified read-only end-to-end in prod. **Open: P2** (interactive column-mapper).* When a feed goes stale (e.g. **Luxury Card = Barclays**, per CR035), upload the bank's own CSV as **`source='manual'`** inside the **bank-feed microservice** (Option A — single ingestion funnel; realizes [CR021](CRs/CR021_BANK_FEED_SERVICE.md) Phase 4). Imports **only new rows** (promote dedup generalized to **any-source overlap**, scoped to manual rows so the live-feed path is unchanged; synthetic `external_id` + file-local occurrence index → idempotent re-upload) and **reconciles to the statement's stated balance** (non-zero drift = loud pre-commit gate). Preinstalled **Barclays profile**; UI = per-row "Upload statement" on Balance Reconciliation. Live run on the real stale card: 46 parsed, 29 already-in-ledger / 17 new, drift −194.99. No DB migration. **P2** = interactive column-mapper + saved per-institution profiles; **P3** = ocr-llm mapping-guess + CR035 inline surfacing. v3 track.
...
<a id="cr021"></a>
- **CR021 — [Bank Feed Service](CRs/CR021_BANK_FEED_SERVICE.md)** — *IN-PROGRESS.* Standalone microservice (fintable.io via Google Sheets), live with 3 institutions, 2 consumers (fin + OCME). **Open:** Phase 4 (Excel/CSV upload — realized as [CR036](CRs/CR036_MANUAL_STATEMENT_UPLOAD.md) P1, shipped v3.0.45; P2 mapper open), Phase 5 (gap detection/stale alerts), Phase 6 (admin UI).
...
- [ ] **`/refresh-ps` rename** (2026-07-03): the daily page is nav-labeled "Refresh Feeds" but keeps the PS-era route, component name (`RefreshPS.jsx`), and copy ("PS records in database") post-CR030. Rename route (with redirect), component, and copy to feeds vocabulary.
- [ ] **UX polish batch** (2026-07-03): COA **single**-account delete confirm has no warning body (`pages/COAManagement.jsx` ~L908 — only multi-delete gets `description`); `pages/TransferAnalysis.jsx` ~L655 passes `\n\n` `window.confirm`-style copy into a styled modal (newlines collapse); hide `/ui-preview` ("UI Preview (CR026)", a self-described non-functional mockup) from the user-facing Settings nav.
