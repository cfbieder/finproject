...
<a id="cr040"></a>
- **CR040 — [Forecast Scenario Compare](CRs/CR040_FORECAST_SCENARIO_COMPARE.md)** — *✅ COMPLETE (feature; built 2026-07-10, not yet released).* `/forecast-compare` (FC step 6): baseline A vs comparison B — KPI deltas + full P&L/BS delta grids (client-side diff reconciling with Review, incl. bank running-balance seeding; base-year double-count caught and fixed via live e2e), recharts A-vs-B lines + diverging delta bars (dataviz-validated palettes, light+dark), deterministic "where they differ" commentary, and on-demand local-LLM narrative with follow-ups (aiReview `compareWith` + compare prompt, migration 035; live gateway round-trip verified — correctly explained the $20K-salary → sweep → interest → taxes chain). 13 frontend + 5 backend tests added (116/252 green). **Open:** release/deploy (**apply migration 035 to prod first**); delete the "CR040 Test B" scenario from dev after browser testing; dev-DB drift note: migration 020 was missing on dev's `fc_ai_reviews`, re-applied.
