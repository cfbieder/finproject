...
<a id="cr023"></a>
- **CR023 — [PocketSmith Removal & PS→Fintable Cutover](CRs/CR023_POCKETSMITH_REMOVAL.md)** — *PLANNING (2026-06-03)*. The endgame for the PS→bank-feed migration. **State:** PS is OFF for PKO+Fidelity; all 11 mapped bank-feed accounts are cutoff-gated (`promote_from_date`) and reconcile to `feed_balances` to the cent; duplicate sweep clean. **Open items to plan (one-by-one in a dedicated thread):** (1) re-anchor balance calibration / `ps-anchor.js` from PS `closing_balance` to `feed_balances` — the last live PS tie; (2) stepwise per-account cutover for the rest (mechanism exists: un-ignore → `seed-bankfeed-cutoffs.js` → manual promote); (3) non-PKO/Fidelity accounts (Chase/Delta/entity/OCME) have no bank feed — CR021 Phase-4 dependency; (4) ignored-account disposition (Fidelity Individual, PKO business 5564); (5) the actual PS code/data removal + exit criteria; (6) forecast modules on parent 29 under-state the fed roll-up; (7) optional UI for cutover settings (script-only today); (8) bank-balance reconciliation monitoring (the live cutover gate now that PS is off). **CR020** (lot/cost-basis) kept separate — plan decides pull-forward vs not.
