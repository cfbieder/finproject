...
- [x] Transfer Analysis: Click-to-change transfer type — click any matched pair, unmatched, or manual group row to reassign the transfer category. Matched pairs update both transactions; unmatched updates only the clicked one; manual groups update all members
- [x] Category Source Mappings — `category_source_mappings` table (migration 018) decouples external system category names from internal app names. Sync JOINs now resolve via mapping table instead of `categories.name` directly. One-to-many: multiple external names (PocketSmith, future Quicken) can map to one app category. COA Management edit modal shows "Source Mappings" section for categories (PocketSmith Name + Quicken Name fields). Auto-creates pocketsmith mapping on new category creation. API: `GET /lookup`, `GET/PUT/DELETE /:id/mappings`
- [x] Account Source Mappings — `account_source_mappings` table (migration 019) extends the same decoupling pattern to Asset/Liability/Income/Expense accounts. Sync JOINs resolve account names via the mapping table. Same UI pattern: COA edit modal shows Source Mappings for any account or category that has a mapping. Auto-creates pocketsmith mapping when accounts are added (via `POST /api/v2/accounts` or `coa/add`). API: `GET /lookup`, `GET/PUT/DELETE /:id/mappings` under `/api/v2/accounts`
- [x] COA Management: Account type display capitalized — Asset, Liability, Income, Expense shown capitalized in the tree, edit modal dropdown, and toolbar filter (DB values remain lowercase for compatibility)
...
- [x] Balance Calibration moved to Transactions — extracted Balance Calibration from ProgramSettings (Settings) into a standalone page (`/balance-calibration`) under the Transactions category, appearing after Ledger on the Transactions landing page. ProgramSettings now only contains Budget Defaults.
...
| 2026-04-13 | **Ledger date filter fix + duplicate detection:** Fixed period filter bug — frontend sent `fromDate`/`toDate` but backend expected `startDate`/`endDate`, so date filtering was silently ignored. Backend now accepts both parameter names. Added "Find Duplicates" feature to Ledger toolbar — client-side analysis identifies potential duplicates (same amount & currency, within 3-day window or identical description). Shows warning banner with count, filters table to duplicates only. Toggle button switches between "Find Duplicates" and "Show All". Modified: `transactions.js` (route), `Ledger.jsx`, `Ledger.css`. |
