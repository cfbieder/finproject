**Status:** COMPLETED — **Released v3.0.25 (2026-06-09).** Engine + schema + API + UI shipped to dev + prod; migration `031` applied to both. Engine unit tests 10/10; dev end-to-end verified (two-module scenario: forced shortfall drains primary then cascades into the backup, `Modules` audit column populated, clear path works). — [Plan](../FC_NEXT_STEPS.md#cr017)
...
- **UI** — [`FCModulesEdit.jsx`](../../frontend/src/features/Forecast/FCModulesEdit.jsx): the "Cash Sweep Target" checkbox → a **Sweep Priority** number input (blank = not in set). [`FCModuleManage.jsx`](../../frontend/src/pages/FCModuleManage.jsx) save payload sends `CashSweepPriority`. [`FCCashSweepModal.jsx`](../../frontend/src/features/Forecast/FCCashSweepModal.jsx) renders the new **Modules** column (source/destination per action) as text.
