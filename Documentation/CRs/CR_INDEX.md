...
| [CR040](CR040_FORECAST_SCENARIO_COMPARE.md) | COMPLETE (unreleased) | v3 | Forecast Scenario Compare | `/forecast-compare`: pick two scenarios, delta KPIs + P&L/BS delta tables (client-side diff reconciling with Review incl. bank running balance), recharts A-vs-B lines + diverging delta bars (validated palettes), deterministic movers commentary + on-demand local-LLM narrative with follow-ups (aiReview `compareWith`, compare prompt, no action blocks; migration 035 `compare_scenario_id`). Built & live-verified on dev 2026-07-10; apply 035 to prod before deploy. |
