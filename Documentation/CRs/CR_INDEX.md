...
| [CR023](CR023_POCKETSMITH_REMOVAL.md) | PLANNING | PocketSmith Removal & PS→Fintable Cutover | Endgame: finish migrating every account off PocketSmith (PS now OFF for PKO+Fidelity, all 11 mapped accounts cutoff-gated + reconciling to the bank) and retire PS. Gathers the post-CR024 open items: re-anchor balance calibration to `feed_balances` (last live PS tie), stepwise per-account cutover, non-PKO/Fidelity feeds (CR021 dep), ignored-account disposition, the actual PS code/data removal, forecast-on-29, cutover-settings UI, bank-balance reconciliation monitoring. To be planned one-question-at-a-time in a dedicated thread. CR020 (lot/cost-basis) kept separate. |
| [CR024](CR024_FIDELITY_FEEDS.md) | OPEN | Fidelity Feeds: market-value balances + investment-activity cash flow | Stands up the 6 Fidelity (SnapTrade) feeds CR022 left ignored. **Phase 1:** `feed_balances` read-override for correct brokerage market-value balances (no lot tracking). **Phase 2:** import + categorize the activity stream by SnapTrade `activity_type` (interest/dividend income; per-account `trade_treatment` for option-vs-equity buy/sell; contribution/withdrawal as matchable transfers; net-zero plumbing suppressed). Single CR; Phase 2 gated on a `Finance → bank-feed` contract change (additive `activity_type`). Migration `025_fidelity_feeds.sql`. Resolves Known Issue #4. Lot/cost-basis stays CR020. |
