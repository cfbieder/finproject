...
| [CR040](CR040_FORECAST_SCENARIO_COMPARE.md) | PLANNED | v3 | Forecast Scenario Compare | `/forecast-compare`: pick two scenarios, delta KPIs + P&L/BS delta tables (client-side diff of `/entries`), recharts A-vs-B visuals, hybrid commentary (instant deterministic movers + on-demand local-LLM narrative with follow-ups via extended aiReview). No migration. |
