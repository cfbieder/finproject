**Status:** OPEN — Planning · Single CR, two phases shipped together · Phase 2 gated on a `Finance → bank-feed` contract change — [Plan](../FC_NEXT_STEPS.md#cr024)
...
| Fixed Income | **31 Fidelity Bond** (−$314,299, 604 tx, computed ~$899,842 → corrected to $1.20M) | resolved (§9 Q1); 29 "Fidelity Fixed Income" is an empty shell |
...
**Read-override.** A mapped Fidelity account flagged `balance_from_feed=TRUE` has its as-of balance computed as `latest bankfeed_balances.balance WHERE balance_date <= asOfDate` instead of `opening_balance + Σ(tx)`. Applies only for as-of dates ≥ the earliest cached balance (2026-05-30); **for earlier dates the existing additive value renders unchanged** (no fabricated history — per §9 Q-history). Blast radius: `balanceSheetFetcher.js` and the as-of-date balance reports (Balance Sheet, Balance Trends, Balance Sheet Periods) that call it. Forecast base values already use a separate hand-entered/broker-reported `market_value` field (decoupled from the tx stream since 2026-05-04), so the forecast path is unaffected.
...
## 9. Resolved mapping decisions + pre-existing cleanups
- **Q1 — Fixed Income mapping → RESOLVED: 31 Fidelity Bond.** The active account (604 tx, computed ~$899,842 → corrected to feed's $1.20M); 29 "Fidelity Fixed Income" is an empty shell ($0, 0 tx). *Pre-existing oddity (not CR024 scope):* 2 forecast modules reference the empty 29 while the real data lives on 31 — flag for separate cleanup.
- **Q2 — Options income category → RESOLVED: 76 "Option Trade"** (income). *Pre-existing oddity:* a near-duplicate 219 "Options Trading" (income) exists — flag for separate dedup; CR024 uses 76.
