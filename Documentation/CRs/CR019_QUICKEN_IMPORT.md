...
- **Bulk-create-and-map: role-aware + transactional (found 2026-05-30, §21).** `coa/add` creates the leaf + a `pocketsmith` mapping before the role-validated quicken mapping, so bulk-creating `target_only` names under a BS parent orphans the leaf when the mapping 400s. Make bulk-create role-aware (for `target_only`, restrict the parent picker to Transfers / create `is_transfer` leaves, or skip leaf-creation and map straight to `Transfer - Historical`) and make create+map transactional (delete the leaf if the mapping fails). Also reconsider `coa/add` hardcoding `source='pocketsmith'` when invoked from the Quicken flow.
