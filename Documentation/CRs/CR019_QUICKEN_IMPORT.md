...
- **Re-validate stored mappings against current roles (found 2026-05-30, §21).** Global `account_source_mappings` rows persist across pivots; stale pre-pivot `target_only`→BS mappings show as "mapped" and are only *warned* about (Q3 banner), not blocked at promote. Add a pass that flags/clears role-invalid stored mappings before the real backfill.
- **Isolate the quicken jest suite from real dev mappings (found 2026-05-30, §21).** The suite runs against the shared dev DB and its `afterEach` deletes global `quicken` mappings by fixture name, clobbering same-named real mappings. Namespace fixture `external_name`s, or run the suite in a transaction/throwaway schema.
...
- **2026-05-30** — **End-to-end walkthrough of the 1→1 model on dev (pko.QIF) + auto-match-at-promote REMOVED.** Validated parse → role-aware mapping → bulk-create → pre-flight → promote → rollback against the PLN pko.QIF sample. Parse/roles/cutoff/calibration/rollback all behaved correctly; full row reconciliation held (2711 staged = 136 split-parents skipped + cutoff-dropped + inserted). Two real bugs found, plus the auto-matcher redesign:
  ...
  - ⚠️ **Two operational notes:** (1) the jest quicken suite runs against the **shared dev DB** and its `afterEach` deletes global `quicken` mappings by fixture name (`Bank Fees`, `Int Inc`, `Int Exp`, `Mortgage - PKO_Bruzdowa`, …) — running it mid-backfill clobbers same-named real mappings. (2) Stale pre-pivot mappings persist in the global `account_source_mappings` table and show as "mapped" (green) even when role-invalid; the Q3 banner warns but does not block promote. Consider a "re-validate stored mappings against current roles" pass before the real backfill.
