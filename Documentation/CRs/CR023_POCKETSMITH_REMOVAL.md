**Status:** IN-PROGRESS — **cutover engine live in prod (v2.15.0–v3.0.10); feed-side migration complete (28 fed / 2 manual)** — early cutovers PKO ×6, Fidelity ×5, Caixa EUR, Luxury card 62, with the US-card/Wise/Revolut tail since folded in (per [CR030](CR030_AUTOMATED_PS_RETIREMENT.md)). **Remaining: the deferred PS-code-removal tail** (2 manual accounts via CR025; exit criteria below). Scope settled 2026-06-03; symmetric cutoff + source-aware calibration/monitoring shipped, plus a per-feed institution filter on the recon table (v3.0.10, 2026-06-06) for working the backlog institution-by-institution (see §0.1 build log + [CR023_PS_MIGRATION_TRACKER.md](CR023_PS_MIGRATION_TRACKER.md)). **Actual PS removal is a deferred tail** (criteria defined here; executed in a future CR when the last account is off PS). — [Plan](../FC_NEXT_STEPS.md#cr023)
