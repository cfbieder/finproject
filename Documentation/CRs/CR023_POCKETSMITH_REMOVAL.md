**Status:** IN-PROGRESS — **cutover engine live in prod (v2.15.0–v3.0.10); per-account PS→Feeds migration ongoing** (13 accounts cut over: PKO ×6, Fidelity ×5, Caixa EUR, Luxury card 62). Scope settled 2026-06-03; symmetric cutoff + source-aware calibration/monitoring shipped, plus a per-feed institution filter on the recon table (v3.0.10, 2026-06-06) for working the backlog institution-by-institution (see §0.1 build log + [CR023_PS_MIGRATION_TRACKER.md](CR023_PS_MIGRATION_TRACKER.md)). **Actual PS removal is a deferred tail** (criteria defined here; executed in a future CR when the last account is off PS). — [Plan](../FC_NEXT_STEPS.md#cr023)
