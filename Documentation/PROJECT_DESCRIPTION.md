...
- **Shared selectors**: `HierarchyFilter` (two-stage cascading filter with group pills + drill-down checklist, right-click solo-select), `CategorySelector` (COA-hierarchy, searchable — used on Budget Worksheet), `AccountSelector` (currency-grouped, searchable — used on Budget Worksheet), `PeriodSelector` (preset-based periods) — reusable across pages
