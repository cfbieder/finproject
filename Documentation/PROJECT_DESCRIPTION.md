...
| `/ledger` | Ledger | Transactions | Account ledger report. Select a single asset or liability account from a COA-tree hierarchy selector to view all its transactions with a running balance column. Prominent account banner shows selected account name and currency. Collapsible filter panel with COA-structured account picker and optional period filter. Supports search, sortable columns, row selection with bulk edit (description/category) and delete, export to Excel, and inline "Add" button to post new manual transactions. Running balance computed client-side from chronological transaction order. Uses `LEDGER_CONFIG`/`LEDGER_EDIT_CONFIG` in `transactionConfig.js` and reuses `GET /api/v2/transactions` with account filter. |
| `/fx-options` | FXOptions | Forecasting | Forecast FX assumptions (budget rates at `/budget-fx`) |
| `/coa-management` | COAManagement | Settings | Chart of accounts CRUD with tree view (expand/collapse hierarchy), horizontal toolbar (search, filters, add), inline row actions on hover (edit, delete, add child), PS analysis, quick-add for accounts and categories. "Add as category" toggle in Add modal for creating categories anywhere in the hierarchy. Components: `COAManagementToolbar.jsx`, `COATreeTable.jsx`, `COATreeRow.jsx`, `COAEditModal.jsx`. |
