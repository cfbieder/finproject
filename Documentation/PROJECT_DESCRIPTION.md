...
| `/trans-actual` | TransActual | Transactions | Actual transactions browser with collapsible filter bar (PeriodSelector, CategorySelector, AccountSelector), description search, value range filters, date format mm/dd/yy, optimized column widths (noWrap amounts, ellipsis description). Action buttons (Edit, Split, Neutralize, All, Delete, Clear Filters, Export) in a separate row below the filter grid with divider border. Edit modal supports Date, Description, and Category (Amount, Currency, Account are PS-sourced and not editable). Uses hierarchical `CategorySelector` via `plTree` prop. Per-row Split and Neutralize action buttons. Selection checkboxes retained for bulk Edit/Delete. **Split:** Per-row button opens split modal. Uses `POST /api/v2/transactions/:id/split`. **Neutralize:** Per-row button creates offsetting entry for brokerage security trades. Uses `POST /api/v2/transactions/:id/neutralize`. |
