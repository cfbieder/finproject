...
export default function TransActual() {
  ...
  const handleNeutralizeClick = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        Rest.buildUrl(`${config.endpoint}/${id}/neutralize`),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
  ...
  }, [selectedRows, config.endpoint, handleSuccess, showSuccess, showErrorToast]);
    ...
    <>
      <main className="page-main trans-budget-main">
        <TransactionFilterActual
          ...
          canNeutralize={selectedRows.size === 1 && !isNeutralizing}
