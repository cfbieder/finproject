**Status:** IN-PROGRESS — **cutover engine live in prod (v2.15.0–v3.0.10); feed-side migration complete (28 fed / 2 manual)** — early cutovers PKO ×6, Fidelity ×5, Caixa EUR, Luxury card 62, with the US-card/Wise/Revolut tail since folded in (per [CR030](cr-030-automated-ps-retirement.md)). **Remaining: the deferred PS-code-removal tail** (2 manual accounts via CR025; exit criteria below). Scope settled 2026-06-03; symmetric cutoff + source-aware calibration/monitoring shipped, plus a per-feed institution filter on the recon table (v3.0.10, 2026-06-06) for working the backlog institution-by-institution (see §0.1 build log + [cr-023-ps-migration-tracker.md](cr-023-ps-migration-tracker.md)). **Actual PS removal is a deferred tail** (criteria defined here; executed in a future CR when the last account is off PS). — [Plan](../current/project-roadmap.md#cr023)

# CR023 — PocketSmith Removal & PS→Feeds Cutover

**Created:** 2026-06-03 (stub) · **Planned:** 2026-06-03
**Follows:** [CR021](cr-021-bank-feed-service.md) (bank-feed service), [CR022](cr-022-bank-feed-parallel-import.md) (PKO parallel import), [CR024](cr-024-fidelity-feeds.md) (Fidelity feeds — DONE).
**Related (kept separate per §0 decision):** [CR020](cr-020-stock-investment-module.md) (investment lot/cost-basis).
...
- **2026-06-06 — per-feed (institution) filter on the recon table (v3.0.10).** Added a **Feed** dropdown to `<BalanceReconciliation/>` so recon rows can be filtered to one institution (Chase, PKO, Fidelity, Amex, Wise, …); the unreconciled count respects the active filter. `GET /balance-recon` now enriches each row with `institution` via `buildExternalIdToInstitution()` ([`routes/bankFeed.js`](../../server/src/v2/routes/bankFeed.js)) — joins the bank-feed `/v1/accounts` (`external_id→connection_id`) with `/v1/connections` (`id→institution_name`); **best-effort, fail-open** to `Unknown` on a bank-feed outage. UI + read-path only, **no migration**. A usability aid for working the per-account cutover backlog institution-by-institution.
...
- **bank-feed exposes 14 accounts across 3 institutions** (PKO 7, Fidelity 6, Bank Pekao 1 = `OC MEDYCYNY ESTETYCZNEJ (PLN) (8781)`, UUID `466a4ae8-…`). It is a **shared read-only microservice** — the OCME app is a second consumer (consumes Pekao + 1 PKO account; see [OCME_BANK_FEED_IMPORT_GUIDE.md](../guides/ocme-bank-feed-import.md)).
...
> **Live per-account backlog + exit monitor:** [cr-023-ps-migration-tracker.md](cr-023-ps-migration-tracker.md) (owner-confirmed dispositions 2026-06-05; §4 query = the "still PS-dependent" gate).
