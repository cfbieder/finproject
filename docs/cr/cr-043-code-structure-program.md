...
- **N10 — Validation stops at 4 of 16 route files; the biggest write surface (forecast, 26 endpoints) has none.** *Partially addressed (Phase 1.4, 2026-07-11):* `routes/forecast.js` now imports `validate.js`; `PUT /scenarios/:id` gets strict field-whitelist + type checks (was passing `req.body` wholesale to `updateScenario`, which silently dropped typo'd keys). The richer module/incexp write endpoints are **deferred to Phase 2.1** — they read explicit PascalCase fields (so already ignore unknowns) and blind strict-whitelisting would 400 real UI payloads until each form contract is enumerated. (S–M remaining)
