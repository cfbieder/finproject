**Status:** DONE (both phases on prod) — **Phase 1 (v2.11.0)** market-value balances + **Phase 2 (v2.13.0, 2026-06-03)** investment-activity cash flow, activated via a per-account cutover cutoff (zero double-count). Remaining: review-queue triage of the promoted Fidelity rows; CR020 still owns lot/cost-basis. — [Plan](../current/project-roadmap.md#cr024)
...
**Created:** 2026-06-02
**Follows:** [CR021](cr-021-bank-feed-service.md) (bank-feed service), [CR022](cr-022-bank-feed-parallel-import.md) (cash-side parallel import — Fidelity was explicitly scoped *out*).
**Relates to:** [CR020](cr-020-stock-investment-module.md) (lot-level investment module) — CR024 is the cash-flow-grain + market-value-balance layer; it does **not** do lot/cost-basis tracking, which stays CR020. Resolves FC_NEXT_STEPS Known Issue #4 (Fidelity brokerage balances wrong; fix is to feed from `feed_balances`).
...
## 9.1 Post-activation: dedup cleanup + PKO cutoff lock-in (2026-06-03)

A review-queue sanity check found **9 duplicate uploads, all on PKO VISA Infinity CB** — bank-feed re-imports of recurring identical charges (−26 ASW near-daily, −28.25 LOT, −8.50) that PS already had accepted, which the R2 dedup's conservative tie-break couldn't disambiguate (the CR022 §G watch item materialising; bank-feed descriptions are reformatted/doubled). Fixed durably by applying the R2 link retroactively (stamped `bank_feed_external_id` on the accepted PS row, re-pointed the staging row, deleted the dup). **Fidelity was clean — 0 duplicates** (the cutoff prevents overlap by construction; 60 rows / 60 distinct external_ids; the lone same-amount coincidence was a redemption + a CD-interest, distinct events).

Since PS is now **turned off** for PKO (and Fidelity), the cutover cutoff was generalised to PKO so safety is by-construction rather than incidental: new **`seed-bankfeed-cutoffs.js`** (set-once `promote_from_date` = PS-last-tx + 1 for every mapped, non-ignored bank-feed account) — the reusable controlled-switchover tool. Applied on prod: all 11 mapped accounts now carry a cutoff (PKO 4→2025-03-28, 12→2026-04-14, 18→2026-05-29, 19/67/69→2026-05-30; Fidelity kept). PKO had 0 unpromoted staging, so this only governs future rows. Net effect: the dedup tie-break can no longer mis-fire on recurring charges (no overlap to merge), and PS removal (CR023) is a no-op for these accounts.
