...
Beyond cash transactions, this CR is also the natural place to land the **investment data model** (per [CR020](cr-020-stock-investment-module.md) §14). Importing 20 years of Buy/Sell/Div records is the largest, most opinionated population of `security_lots` / `security_transactions` the system will ever do — building the schema here means CR020 starts with a working dataset rather than empty tables.
...
Created in the same migration to keep CR020 unblocked. Full column lists and indexes per [CR020 §4](cr-020-stock-investment-module.md#4-data-model-owned-by-cr019-referenced-here):
...
- `accounts` — adds new `skip_transfer_analysis BOOLEAN NOT NULL DEFAULT FALSE` column. Set to `TRUE` on transfer-flagged leaves that don't have a matching pair (notably "Return of Capital"), so they don't appear as perpetually-unmatched in [/transfer-analysis](../current/project-description.md). See §5.3 for usage on RoC.
...
- **1→1 transfer model (pivot, see §8.2 for full rationale).** Each transfer row in `quicken_staging` becomes a **single** transaction on the origin BS leaf with a transfer-category category. No target-side fanout. Cross-currency pairs are recognized post-hoc by Transfer Analysis's `base_amount` matching with category-aware tolerances (1% for `Transfer - FX`, exact for others), run **manually by the user after promote** — promote itself does no matching (see §6.4 step 10 and §21, 2026-05-30).
...
2. **Insert direct cash transactions** from `quicken_staging` — **1→1 model**. Each non-parent staging row produces exactly one row in `transactions` on the origin's mapped BS leaf:
   ...
   - **Transfer** (`transfer_target_account IS NOT NULL`): one row only — `account_id = origin BS leaf, amount = row.amount`. The category resolution is role-aware:
     ...
     - **No target-side row is created.** Pairs that span QIF files are recognized post-hoc in a manual Transfer Analysis run, not by fanout and not at promote. This eliminates the cross-currency bug where a single staging row's currency leaked onto the target leaf.
...
10. **~~Auto-match transfers (Q5 pivot addition).~~ REMOVED 2026-05-30 — see §21 walkthrough.** Promote no longer runs any transfer matching. The Transfer Analysis matcher has **global scope** (it considers every `is_transfer=TRUE AND skip_transfer_analysis=FALSE` transaction in the batch's date window, all sources) and persists `transfer_matched=TRUE` on the pairs it finds — including pure PS↔PS pairs unrelated to the import. Because **`runRollback` only resets this batch's rows**, any flip on a non-batch row was **permanent** — a promote→rollback cycle silently mutated PS-era transfer state across the whole date window. (The original implementation also never actually matched anything: it filtered `transfer_matched = FALSE` while inserting rows as `NULL`, so `NULL = FALSE` excluded them all; fixing that filter is what exposed the irreversibility problem.) Matching is now **deferred to the existing manual `/api/v2/transactions/transfer-analysis`** — idempotent, re-runnable, and decoupled from the batch lifecycle. Inserted transfer rows land with `transfer_matched=FALSE`; the user runs Transfer Analysis post-promote when ready. Promote stays purely additive and cleanly reversible.
...
The cross-currency check (different base currencies on the leaf accounts) is critical for matching: only `Transfer - FX` gets the 1% tolerance in Transfer Analysis, so cross-currency rows landing in `Transfer - Bank` would never match.
...
- Pre-2022: investment-account historical valuation = `SUM(open_lots × close_price_at_date)` computed live from `security_lots` + `security_prices`. Balance Trends ([CR020 §9.2](cr-020-stock-investment-module.md#92-balance-trends-report-cr018)) drills via this path when a single investment account is selected.
...
This CR commits to every item in [CR020 §14](cr-020-stock-investment-module.md#14-notes-for-the-cr019-quicken-import-author):
...
- ~~**Bulk-create-and-map: role-aware + transactional (found 2026-05-30, §21).**~~ **DONE 2026-05-30.** `BulkCreateModal` now (a) computes per-row `bulkEligibility(role, parent)` and **skips rows whose role doesn't fit the chosen parent** before creating anything (target_only names are never leaf-created — they map to a Transfer leaf via the picker), and (b) makes create+map **atomic** — if the role-validated mapping is rejected, the freshly-created leaf is soft-deleted (only when newly added, not a reactivated account). The modal shows a per-row "will create / skip — reason" preview, a skipped-count warning, and gates the Create button on the eligible count. Remaining minor: `coa/add` still hardcodes `source='pocketsmith'` — acceptable now since no orphan leaf survives a failed map; revisit only if it causes PS-import collisions.
- ~~**Re-validate stored mappings against current roles (found 2026-05-30, §21).**~~ **DONE 2026-05-30.** `findRoleInvalidMappings(client, batchId)` (in `quicken-promote.js`, exported) flags stored `quicken` mappings that would **corrupt** a promote given the name's role: `origin`/`both`→non-BS, and `category`→non-P&L. (`target_only` is intentionally not flagged — promote handles any target via direct transfer-leaf use or §8.2.3 derivation; a stale `target_only`→BS is surfaced by the mapping panel's Q3 banner instead.) `runPromote` refuses fail-loud on any hit; the pre-flight API returns `roleInvalid[]` and sets `canPromote=false`; the Pre-flight UI shows a blocking alert. Live-verified + unit-tested.
- ~~**Isolate the quicken jest suite from real dev mappings (found 2026-05-30, §21).**~~ **DONE 2026-05-30.** Promote tests now parse a dedicated `Samples/quicken/fixtures/cash_isolated.QIF` whose category/transfer names are `_qpr `-namespaced (and account name `cash_isolated`), so the authored `quicken` mappings can never collide with real global mappings on the shared dev DB. (Parser tests keep using `cash_simple.QIF`.)
...
**Cutover model (decided 2026-06-02): re-run pipeline on prod (NOT dump/restore).** Prod is rebuilt deterministically — apply migration 022 + seed containers/income-leaves/FX, copy `quicken_*` staging+mappings dev→prod (or re-import QIFs), re-promote batches, **then run `ps-anchor.js --apply` on prod** (it computes from prod's own PS data — no hardcoded ids). Corollary rule for all remaining CR019 data work: **no manual SQL, no hardcoded dev ids — every fix is an idempotent script validated on dev and listed as a cutover step.** (The Fidelity 635 manual-SQL handoff has since been scripted — see §22.3.)

**DIVERGENT follow-ups — RESOLVED 2026-06-02:** (a) **Brokerage** is NOT a mark-to-market anchor decision — the Fidelity accounts are a *half-finished trade-neutralization* (the `neutralize()`/`auto-offset` button applied to only 7 of thousands of trades) + missing opening balances. Anchoring down to `closing_balance` would erase real holdings (Bond −$314k). Correct values already exist in the bank-feed `feed_balances` (daily); fix = feed integration (map Fidelity feed accounts + balance sheet reads `feed_balances`), routed to the bank-feed/CR022 thread (see `FC_NEXT_STEPS.md` Known Issue #4). Brokerage accounts are now excluded from `ps-anchor` (it skips accounts under `BROKERAGE_CONTAINERS` — Fidelity Stock / Fidelity Fixed Income — reporting them FEED-OWNED and never writing; the stray `ps-anchor` row on acct 26 is left intact, to be superseded by the feed wiring). (b) **DIVERGENT credit-card gaps are NOT missing/dup rows** — the order-independent day-level test (`B(D)=opening+cumΣamount` must appear in each day's closing_balances) passed 0/798 and 0/469 for the two PKO cards: data is complete, the gap is purely PocketSmith's unreliable intra-day ordering making the "latest closing_balance" pick wrong. `computed` is already correct; do not anchor them.

## 22.3 `retire-handoff.js` — scripted historical-account handoff (2026-06-02)
...
**STEP 1 — Code on prod (REQUIRED — not yet shipped).** Deploy a fresh release from `main` via `Scripts/deploy-to-production.sh` (prod's `v2.8.2` lacks value-only promote + PS-anchored calibration — see Model). Confirm `quicken-promote.js` on prod has no investment guard + PS-anchored `recalibrate`.

**STEP 2 — Seed prod COA (G1, done).** `node server/src/v2/scripts/seed-cr019-coa.js --apply` (dry-run first). Creates the 10 objects by name if absent: `Historical Assets`/`Historical Liabilities` (under Assets/Liabilities roots), `Closed Cash`/`Closed Debt (default)`, `Fidelity (historical)` (under Historical Assets), `Transfer - Historical` + `Return of Capital` (under Transfers), `Financial Income - Dividend` + `Interest Income` (under Financial Income), `Realized Gain (Historical)` (root).
...
- **2026-05-30** — **End-to-end walkthrough of the 1→1 model on dev (pko.QIF) + auto-match-at-promote REMOVED.** Validated parse → role-aware mapping → bulk-create → pre-flight → promote → rollback against the PLN pko.QIF sample. Parse/roles/cutoff/calibration/rollback all behaved correctly; full row reconciliation held (2711 staged = 136 split-parents skipped + cutoff-dropped + inserted). Two real bugs found, plus the auto-matcher redesign:
  - 🔴 **Promote-time auto-matcher was both broken AND unsafe.** It filtered `transfer_matched = FALSE` but promote inserted rows with `transfer_matched = NULL`, so `NULL = FALSE` excluded every row it had just inserted — the matcher silently matched nothing and reported `autoMatched=0, unmatched=0`. Fixing the predicate (`COALESCE(transfer_matched, FALSE) = FALSE`) revealed the deeper problem: the matcher has **global scope** (all transfer txns in the batch's date window, all sources/categories) and persists `transfer_matched=TRUE` on pure PS↔PS pairs unrelated to the import, while **`runRollback` only resets this batch's rows** — so a promote→rollback cycle permanently mutated PS-era transfer state (empirically: 866 non-batch rows flipped; rollback left them flipped). **Resolution: auto-match removed from promote entirely** ([§6.4 step 10](#)). Promote is now purely additive and cleanly reversible; transfer rows land `transfer_matched=FALSE`; the user runs the existing manual `/api/v2/transactions/transfer-analysis` (idempotent, re-runnable) post-promote. `autoMatchTransfers()` deleted from `quicken-promote.js`; promote result no longer returns `autoMatched`/`unmatched`; frontend toast updated to point the user at Transfer Analysis. Regression test repurposed to assert promote does NOT auto-match or touch non-batch rows. Suite now **58 passing**.
  - 🟠 **Bulk-create-and-map orphans COA leaves on role rejection (follow-up, see §19).** `coa/add` ([`util.js:356`](../../server/src/v2/routes/util.js#L356)) always creates the leaf **and** a `pocketsmith`-source mapping *before* the role-validated quicken mapping; for `target_only` names the quicken mapping correctly 400s, leaving an orphan BS leaf + a stray `pocketsmith` mapping with no cleanup. Not silently promoted (batch detail joins `source='quicken'` only), but litters the COA on the real backfill. **FIXED same day:** `BulkCreateModal` is now role-aware (skips ineligible rows before any create — target_only names are never leaf-created) and atomic (soft-deletes the leaf if the role-validated map is rejected). See §19.
  - ⚠️→✅ **Two operational notes, both FIXED same day (see §19):** (1) the jest quicken suite clobbered shared-dev global mappings via its `afterEach` — now isolated by a `_qpr `-namespaced `cash_isolated.QIF` fixture. (2) Stale role-invalid stored mappings were only warned about — now `findRoleInvalidMappings` blocks promote on **corrupting** role mismatches (origin/both→non-BS, category→non-P&L) via `runPromote` + pre-flight `roleInvalid[]`/`canPromote=false`; stale `target_only`→BS stays a non-blocking Q3 banner warning (promote derives it). Suite now **59 passing**.
- **2026-05-29** — **Cash-transfer model pivot — 1→1 with post-hoc matching.** Replaced the original §6.4 step 2 fanout (1 staging row → debit+credit pair) with a single transaction per row, plus auto-run Transfer Analysis at the end of promote. Driver: the fanout leaked the source file's currency onto the target side for cross-currency transfers (PKO PLN → Chase USD), and §8.2 cross-file dedupe couldn't pair them because absolute amounts didn't match. The 1→1 model is the same shape PocketSmith data already uses; `base_amount` matching with category-aware tolerances (`Transfer - FX` 1%, others exact) handles cross-currency pair recognition correctly. Changes landed:
  ...
  - **§6.4 step 11 added** — auto-match via the existing Transfer Analysis logic over the batch's date range; `transfer_matched=TRUE` persisted on auto-matched pairs. **(SUPERSEDED 2026-05-30 — removed; see the entry above. The matcher's global scope made promote irreversible.)**
