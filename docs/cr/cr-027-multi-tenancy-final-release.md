...
- **Token delivery (invite + reset):** **manual / out-of-band is in scope for CR027** — the admin generates the link and hands it to the user (admin-invite means the admin knows every recipient personally, so a transactional email provider is not required at this scale). **A third-party email provider (Resend / Postmark / SES) is deferred to the public-signup CR**, where email verification + self-service reset to strangers make it unavoidable and where fresh-domain deliverability (spam-landing of reset mail) actually bites. To keep that a one-file swap later, **route all sends through a single `sendEmail()` seam** now whose default implementation just returns/logs the link for manual copy — no provider, no API key, no new outbound failure mode in this CR. (The token *generation, expiry, single-use, and session-revocation* logic above is identical regardless of delivery channel — that is the security-critical part, and it ships in full.)
...
- TOTP 2FA. Per-tenant audit log. Email verification / public self-signup (if the SaaS opens up beyond invite). **This is also where the transactional email provider lands** (Resend recommended for low invite-only volume; Postmark if reset-mail deliverability is paramount; SES only at scale) — CR027 ships invite/reset links via manual out-of-band delivery behind a `sendEmail()` seam (see Auth & session security §Token delivery), so adopting a provider here is a one-file swap.

> **⚠️ Critical scoping reality — ongoing feeds are single-owner today.**
> The bank-feed microservice reads **one** owner's Fintable Google Sheet; it has **no concept of per-user bank connections**. So within CR027's scope, the only *ongoing* data paths a new invited user actually has are **Excel/CSV upload and Manual entry** — the **Fintable feed is effectively owner-only** until a separate "per-tenant feed connection" CR builds: (a) each tenant authorizing their own bank/Fintable source, (b) routing feed data to the right `tenant_<id>` schema, and (c) friendly per-tenant **"reconnect needed" / feed-stale** states so users self-serve breakages (Design review #2) without emailing the admin. **Consequence for sequencing:** CR025 (manual entry) + the Excel importer are the real day-one onboarding paths; the new IA must not present "Bank Feed (Fintable)" as available to a tenant who has no way to connect one. Surface it as "coming soon / admin-managed" for non-owner tenants until that CR lands. **UPDATE 2026-08-20 — the upstream half of (a) and part of (b) now has an answer, and this paragraph's "no concept of per-user bank connections" is no longer a statement about Fintable, only about bank-feed.** Fintable shipped **Workspaces**: separate environments under one login and plan, each with its own bank connections, plus **per-credential workspace grants** so one token reaches several workspaces, ticked explicitly and re-checked per request. A workspace per tenant is therefore buildable. **It does not make this cheap:** Office/Enterprise plan, billing stays **combined under the owner** (the owner funds every tenant's volume — not self-serve SaaS), bank-feed still has no workspace/tenant column so the routing is real schema work, and (c) the per-tenant reconnect UX is untouched. **For the single-owner setup it is actively unwanted** — fin's ledger deliberately spans personal finances and the Polish operating companies in one book, which the forecast and balance sheet depend on, so splitting upstream would add a merge step. Recorded so the constraint is not re-derived from a stale premise; see [CR059 §24](cr-059-fintable-api-ingestion.md). **No CR, no action.**
...
- `frontend/src/pages/UIPreview.jsx` + `.css` + the `/ui-preview` route — remove the **in-app** mockup page from the shipped product. **Keep** `docs/cr/cr-026-ui-preview.html` — it is promoted to the **living style reference** (see §"Design / style reference"), not deleted.
