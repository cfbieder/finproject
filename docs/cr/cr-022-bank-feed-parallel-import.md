**Status:** PHASE G (parallel-run observation, ≥1 month) — Phases A–F **DONE**. Phase F executed in prod 2026-06-02 (STEP 1–8): migrations 023+024 applied (pg_dump bracket), v2.8.2 deployed + healthy, smoke 7/7, 6 PKO accounts mapped / 7 ignored, first promote seeded the ledger (134 PS rows linked, 32 bank-feed rows in review queue), and **the day-one reconciliation reads `total_ps_only=0` with 143 matched** after the fintable sheet history was deepened to cover the 30-day window. Stage-only daily cron added (06:00, prod host). **Now observing** per §G — watch `ps_only` holds at 0 for ≥1 month before opening the PS-removal CR (CR023). — [Plan](../current/project-roadmap.md#cr022)

# CR022 — Bank Feed Parallel Import (Additive Second Source)

**Created:** 2026-05-30
**Follows:** [CR021](cr-021-bank-feed-service.md) (Bank Feed Service — the upstream service this CR consumes).
...
- **Anything in CR023+.** PS code removal, fin v3 namespace, contract-versioning to v2 — all future CRs.
  - **CR023 scope item — re-anchor Balance Calibration to the bank feed (logged 2026-06-02).** `/balance-calibration` (and `ps-anchor.js`) currently calibrate `opening_balance` against PocketSmith's `closing_balance`. That anchor is the source being retired and is already going stale (PKO PS feed observed down 3–4 days). The function stays essential — fin balances are `opening_balance + Σ(amount)` and no account has history to inception (bank-feed starts ~mid-May; entity accounts like OCME aren't fed at all), so calibration is load-bearing for balance-sheet accuracy (it's why the day-one reconciliation matched the bank to the cent). **CR023 should switch its anchor to the bank's reported balance (`feed_balances`) for mapped accounts**, keeping PS as a fallback only for unmapped/PS-only accounts. Placement note: it's occasional setup/maintenance (a sibling of Bank Feed Setup, not the daily Refresh→Review→Ledger flow) and pairs naturally with the reconciliation panel (detect drift → calibrate to fix). **Priority raised to near-term (2026-06-02):** it is now the owner's intended **monthly mark-to-market** tool for Fidelity investment accounts (brokerage `feed_balances` = market value; calibrate `opening_balance` monthly so fin = market value — `opening` acts as the unrealized-gain plug). It must anchor to the feed for that, since PS has no Fidelity market values. **Prod incident handled same day:** a CR019 Quicken backfill landed on prod (added 2014–2022 history to PKO) and Fidelity got mapped+promoted by another thread, leaving balances badly mis-calibrated (PKO ~+110k; Fidelity Bond −304k / Cash Mgt +253k / Stocks −62k). Recalibrated **PKO 5647 (cash)** opening `115,478.38→5,534.68` to the live `feed_balances` (it read 196,050 vs the bank's 86,106 after the quicken backfill) — PKO/PS-cash assets reconcile to the cent, liabilities by sign-convention. The **5 Fidelity brokerage accounts are valued by CR024's `balance_from_feed` read-override** (balance sheet already shows market value); a brief `opening_balance`-plug MTM applied here *before that mechanism was understood* was **reverted** (Fidelity openings restored). Going-forward investment MTM = Unrealized-G/L *entry* (CR023 decision log 2026-06-03), not an opening plug. Backup `~/fin-prod-pre-pko-calib-20260602.dump`. The repeatable version of this manual SQL is the feed-anchored Balance Calibration tool. **Cross-thread note for CR019:** verify the Quicken prod cutover's opening-balance recalibration — it desynced PKO until re-anchored to the feed.
...
**Gate adjustment (2026-06-02, prod finding): balance reconciliation against the BANK is now the PRIMARY gate; `ps_only` is secondary.** During the day-one prod check, PS's PKO feed was found to be **down ~3–4 days** (latest PS tx: PKO 05-28, VISA CB/KB + Savings 05-29; PKO-USD stale ~14 months, EUR ~7 weeks) while bank-feed was current to 06-01. That makes `ps_only=0` *unreliable* for the recent window — PS has nothing there to be "missed", so the 0 is partly an artifact of PS being blind, not of bank-feed being complete (the §G footnote scenario, now real). Since PS unreliability is the very thing this project exists to fix, **`ps_only` cannot be the sole cutover criterion.** Lead instead with a **PS-independent balance check**: per mapped account, compare fin's computed balance (`opening_balance + Σ amount`) to the bank's reported balance (bank-feed `feed_balances`). Day-one result: **PKO, PKO-USD, PKO-EUR, PKO-Savings matched the bank exactly**; VISA KB matched (sign convention — liability stored negative); **VISA CB was off by 406.04 PLN — investigated + explained:** the 11 bank-feed-only VISA CB charges (all 05-30/06-01) sum to 404.71, ≈ the full gap; the bank's reported card balance has been flat at 15,373.74 since 06-01. So fin is *ahead of* the bank's posted card balance by recent charges it hasn't statemented yet — benign timing (the good direction), expected to converge to 0. **Phase-G watch item (secondary):** recurring identical small charges (−26.00/−28.25/−8.50) trip the §8 conservative "don't-merge-when-ambiguous" rule, leaving both PS+bank-feed copies; harmless now but could slowly double-count on a card — if VISA CB's gap *grows* rather than converging, investigate there (definitive check = the VISA CB bank statement, transaction-by-transaction). Revised cutover criterion: every mapped account's fin balance reconciles to the bank's reported balance (within a small pending-timing tolerance) AND `ps_only` shows no *persistent* gap during whatever windows PS is actually live.
...
Append to `docs/current/test-overview.md` §"Running everything before a release":
...
| 2026-06-02 | **Phase F cutover done; first promote `ps_only`=0 only after deepening the fintable sheet** | Prod cutover ran clean (migrations→deploy→map→promote→cron). The 30-day first promote initially showed `total_ps_only=53` — diagnosed as a backfill-depth artifact (bank-feed history started mid-May, so the old edge of the window predated the feed; recent rows all matched, so forward coverage was fine and those PS rows were already safe in the ledger). It would have self-resolved as the rolling window advanced, but the owner deepened the fintable sheet to ~05-01, re-synced bank-feed (`POST /v1/sync`, 329→545 tx), and re-pulled (`/refresh sinceDays=35`): `ps_only` dropped to **0** with 143 matched, +51 links, only +2 new rows — confirming the extra history was almost all PS-duplicate (dedup absorbed it, no queue flood). Lesson: deepening the source sheet is a safe, optional accelerant for a clean day-one gate, *provided* dedup link-rate stays high (watch new-row count). |
| 2026-06-02 | **Category suggestions from history (Approach A: merchant-history rules)** — shipped v2.10.0 | Bank-feed rows land uncategorized; rather than ML or the local LLM, suggestions reuse the 27.8k accepted+categorized history. `services/categorySuggest.js` derives a merchant key per `description1` (dedupe bank-feed's whole-string doubling, strip digits/IBANs/location tokens, keep leading 3 tokens) and returns the dominant category for that key (≥2 samples, >50% majority; else null). `POST /transactions/category-suggestions` is read-only; a **Suggest categories** button fills uncategorized review rows as *pending* (never auto-accepts — a wrong auto-fill is worse than none), so it's reviewed before Accept; self-improving as rows are accepted. Chosen over LLM (B) / hybrid (C) for determinism, explainability, and zero infra. Prod check: 18/28 rows got confident suggestions, all sensible; "orange" (telecom) matched across PS formats while "orange festiwal" stayed separate. Caveat: suggestions *mirror past choices*, so historical mis-categorizations propagate — pending-not-accepted is the guard. Future: LLM fallback for no-history merchants (→ C); a functional index on the merchant key if the per-call history scan gets slow. |
...
|---|---|---|---|---|
| G1 | **Fin-side scheduled trigger.** Step ① has two pulls: Sheet→bank-feed (bank-feed cron, ✅) and bank-feed→fin (was manual-only). | High | small | ✅ **DONE** (`e975c0f`): new `POST /ingest-bank-feed/ingest` (STAGE-ONLY — never promotes) + `Scripts/refresh-bank-feed.sh` for host crontab (per project convention; auto-stage, promote stays behind the button). 2 route tests. ✅ **Crontab added on prod host 2026-06-02** (`0 6 * * *`, stage-only, `BASE_URL=http://localhost:3005 SINCE_DAYS=14`); test-run staged 255 rows (new=0, idempotent), exit 0. |
...
| G3 | **Bank-feed rows surface in the PS review page (`RefreshPS.jsx`).** Works (queue is source-agnostic) but is a UX smell — bank-feed rows under a page named "Refresh PS". The optional `RefreshBankFeed.jsx` sibling (§3.2) was not built; instead the `/bank-feed-diagnostic` page (now "Refresh Bank Feed", in the Transactions hub) is the operational surface. Categorize/accept still happens on the PS page. | Medium (UX) | med | ✅ **DONE** (2026-06-02, `eaa65fc`, deployed): added a **Source column** (PS / Bank feed / Quicken) to the review-queue `TransactionTable`, shown only in the review view (`logPrefix=ReviewNew`); ledger/actuals unchanged. Resolves the "which source is this row?" ambiguity. Label kept generic ("Bank feed", not "FT") per §3.3. Desktop done; `MobileRefreshPS.jsx` not updated (follow-up if mobile review is used). |
...
| G7 | **Prod migration state unverified + manual.** Local prod-profile DB has only through 022 (no 023/024). Migrations are manual (deploy script doesn't run them). Must verify + apply 023+024 on the real prod box before the feature works there. | High (for function) | small | ✅ **DONE** (2026-06-02): Phase F STEP 1–3 executed. Prod (`fin-postgres` :5433, which runs on **this same host** — 192.168.1.87 is bound to `enp1s0` here, so host-local `docker exec` reaches prod directly) was confirmed at 022; pg_dump taken (`~/fin-prod-pre-cr022-20260602.dump`, 2.9M); 023+024 applied clean (atomic, verify-blocks passed); re-checked — all objects present, `account_id` nullable. STEP 4–8 also completed same day (deploy v2.8.2, smoke 7/7, mappings, first promote ps_only=0, cron). |
