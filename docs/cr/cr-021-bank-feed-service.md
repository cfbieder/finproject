...
**Created:** 2026-05-28
**Supersedes:** [CR014](cr-014-pocketsmith-replacement.md) (dual-provider in-app integration) and obsoletes [CR015](cr-015-ps-reexport.md) (PS re-export — irrelevant once PS is removed).
...
| 2026-05-29 | **banksync.io rejected — PKO not in institution catalog** | When attempting to add a bank connection in banksync.io, PKO Bank Polski did not appear in their selectable institution list. Earlier "PKO covered" claim was based on marketing-level coverage info, not a tested connection. **Lesson: no upstream gets committed without a confirmed live PKO link in their product first.** Pivot needed; bank-feed architecture (one-file adapter swap) absorbs this without rework. |
| 2026-05-29 | Upstream candidate shortlist (post-banksync) | Originally believed fintable.io and moneysheets.io to be one company; separately confirmed they're distinct. Shortlist updated: (1) fintable.io — coverage page lists PKO (bank ID 9079); API access tbd, requires validation. (2) GoCardless Bank Account Data direct — PSD2 specialist, free tier, established. (3) moneysheets.io — see below, disqualified. Validation path: actually link PKO in candidate's free tier before writing them into this CR. |
| 2026-05-29 | **moneysheets.io rejected — no developer API** | moneysheets.io is a product-only Google-Sheets-sync tool (€50/yr). Their "How it works" describes a 4-step flow that ends with bank data appearing in a Google Sheet; there are no REST endpoints, no webhooks, no API keys. bank-feed's architecture requires an HTTP upstream we can call directly. Wiring moneysheets in would require reading from Google Sheets as the transport — fragile, polling-only, defeats contract isolation. Coverage of PKO is real but unusable for our purpose. |
| 2026-05-30 | **moneysheets.io reconsidered as Excel/Sheets adapter** | User shared `Samples/Moneysheets/MoneySheets Master Template - SingleCurrency.xlsx`, a real moneysheets export containing 3,868 PKO transactions + 871 balance snapshots — proving (a) PKO connection works, (b) output schema is clean and ingestable (Transactions: 13 columns incl. signed Amount, ISO Currency, stable Transaction ID, Account UUID, settled status; Balance History: per-day snapshots per account). My earlier disqualification overweighted "no API" — bank-feed's design already includes an excel/csv adapter for the Fidelity case; moneysheets fits that pattern. €50/yr is cheap. **Now a viable candidate.** Three delivery paths to evaluate: (a) Google Sheets API read on schedule, (b) auto-export to watched folder, (c) manual upload to `POST /v1/excel/upload`. Open questions: sync cadence at €50/yr tier; whether the Transaction ID stays stable across re-syncs (affects dedup strategy). |
| 2026-05-30 | **Fintable proven: GoCardless underneath, PKO confirmed, full API fidelity in xlsx** | User shared `Samples/Fintable/⚡️💵 Fintable Template.xlsx`. Two sheets — Accounts (7 PKO accounts, all status=READY, with IBAN, BIC, product type, multi-currency PLN/EUR/USD, both forwardAvailable + interimBooked balances) and Transactions (128 real PKO transactions, 8 columns: Date, Amount, Description, Category, Account, Attachment, Transaction ID (composite stable hash), Raw Data). **Raw Data column carries the full GoCardless transaction payload as JSON** — `internalTransactionId`, `bookingDate`, `valueDate`, `bookingDateTime`, `transactionAmount`, `balanceAfterTransaction`, `pending`, `remittanceInformationUnstructured`, `ext_nordigen_acc_id`. **Critical**: `ext_nordigen_acc_id` proves fintable is built on GoCardless Bank Account Data (Nordigen rebrand) — so this file IS the validation that GoCardless direct covers PKO with all the PSD2 fields. Fintable is GoCardless + Excel/Sheets sync layer. Schema is significantly cleaner than moneysheets for our purposes. |
| 2026-05-30 | ~~Upstream decision: GoCardless direct~~ — **GoCardless rejected: new signups for Bank Account Data are disabled** | Plan was: Phase 2a fintable file ingest as bridge → Phase 2b migrate to GoCardless direct API. User confirmed via gocardless.com signup page: "New signups for Bank Account Data are currently disabled." Existing GoCardless customers (fintable, moneysheets) keep access; new users cannot sign up. Closes the "skip the middleman" exit indefinitely. |
| 2026-05-30 | **Upstream decision (final): Fintable + Excel adapter** | Fintable is the chosen upstream. Phase 2 = build Excel/xlsx adapter; ingest the fintable export file (`Samples/Fintable/⚡️💵 Fintable Template.xlsx` proven schema); validate end-to-end on real PKO data. Fintable's value proposition is now stronger than initially assessed: they hold grandfathered GoCardless API access that new users cannot obtain. Long-term dependencies: fintable's business continuity + their continued GoCardless relationship. Ultimate fallback (if fintable disappears) is pure-manual Excel export from PKO's web banking + the same xlsx adapter — degraded UX (manual cadence) but the contract + downstream stay unchanged. Adapter-swap optionality is now partially closed (no "go direct" path), worth recording honestly. |
| 2026-05-30 | Phase 2 scope re-defined | Phase 2 = single deliverable: build excel/xlsx adapter (lives at `src/adapters/excel.js` + `src/converters/fintableToCanonical.js`) + `POST /v1/excel/upload` implementation + auto-watch a fixed dropbox folder for the same file. First measurable result: real PKO transactions queryable via `GET /v1/transactions` and balances via `GET /v1/balances`. ETA 2-4 days. Webhooks deferred indefinitely (no push delivery available without GoCardless direct). |
| 2026-05-30 | **Delivery: Google Sheets API (not xlsx file)** | User confirmed fintable's destination is a Google Sheet in their Drive; the xlsx we analyzed was a manual `File → Download` of that Sheet. So the live Sheet has the same schema (incl. Raw Data column with full GoCardless JSON). Sheets-API-direct is more robust than xlsx file watch — no human-in-the-loop file movement, no folder-watch fragility. Phase 2 adapter switches to `src/adapters/googleSheets.js` using `googleapis` npm package. Service-account auth: user creates Google Cloud service account, shares Sheet read-only with its email; bank-feed reads the Sheet on schedule. Polling cadence: hourly (fintable syncs daily; 1–2h lag is fine). `POST /v1/excel/upload` retained as the manual fallback path and for the future Fidelity case. |
| 2026-05-30 | Phase 2 schema mapping pinned | (a) **bank_connections**: one row for fintable, `source='fintable'`, `institution_id='PKO_BPKOPLPW'`, raw stores spreadsheet metadata. (b) **feed_accounts**: one row per Accounts-sheet row; `external_id=` fintable Account UUID (e.g. `6fbf7e12-...`); name = display string; currency from Currency column; raw = full row + Raw Data JSON. (c) **feed_transactions**: one row per Transactions-sheet row; `external_id=` Transaction ID composite hash; `source='fintable'`; `pending` extracted from Raw Data JSON; `currency` extracted from Raw Data JSON (Amount column is currency-less). (d) **feed_balances**: one row per Account per sync, `balance` from Accounts.Balance, source `interimBooked` per Raw Data. |
