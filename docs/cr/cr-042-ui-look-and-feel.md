...
| U5 | Report consolidation + sidebar re-cut to ≤8 items + user-vocabulary renames (T2) — **Balances 4→1 (v3.0.72, owner-approved) + Cash Flow 2→1 + Budget-vs-Actual 3→1 DONE v3.0.73** (all three via shared `<ReportTabs>`); 2 vocab renames done; ✅ **owner-accepted v3.0.97** (all tabs and all nine old-URL redirects verified against the owner's own bookmarks). Deferred (owner input): calibration→Settings move, "Upload PS" disposition, Forecast-step sidebar collapse | **owner decision**; easier after U4 |
