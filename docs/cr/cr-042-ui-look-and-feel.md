...
**Status:** 🟡 OPEN — **U1–U5 all shipped and owner-accepted** (v3.0.69 → v3.0.97; see §Plan progress). What remains is **owner-input IA only**: the calibration→Settings move and collapsing the six Forecast steps in the sidebar. No engineering work is blocked. *(**"Upload PS" is settled — owner decision 2026-07-13: KEEP, no change.** The question was posed on a wrong premise: the page is not a stray top-level nav item, it already sits in the `Database` category the sidebar renders as **Data Sources**, beside Quicken Import / Bank Feed Setup / Backup Database. Nothing has been imported through it since **2026-06-04** and the feed cutover completed 2026-06-06 — but it stays as a working PocketSmith-CSV escape hatch in case a historical backfill is ever needed. Do **not** delete it or the `/api/v2/ingest-ps` routes.)* Produced by the 2026-07-11 three-lens review (UI · code structure · marketability). Companion CRs: [CR043](cr-043-code-structure-program.md) (code structure), [CR044](cr-044-productization-marketability.md) (marketability).
