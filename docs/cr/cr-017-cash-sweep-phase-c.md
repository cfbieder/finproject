**Status:** COMPLETED — **Released v3.0.25, picker hardening v3.0.26 (2026-06-09).** Engine + schema + API + UI shipped to dev + prod; migration `031` applied to both. Engine unit tests 10/10; dev end-to-end verified (two-module scenario: forced shortfall drains primary then cascades into the backup, `Modules` audit column populated, clear path works). — [Plan](../current/project-roadmap.md#cr017)
