**Status:** COMPLETED 2026-05-20 — [Plan](../current/project-roadmap.md#cr016)
...
- **DRY prerequisites** still on the backlog (`collectCollapsiblePaths` duplicated in `Balance.jsx` + `BalanceChart.jsx`; FX rate lookup duplicated in BudgetInput + transaction modals). These are tracked in [FC_NEXT_STEPS.md](../current/project-roadmap.md) §2 / §4.1; the test framework is in place to cover them once extracted.
