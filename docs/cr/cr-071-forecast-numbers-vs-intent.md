...
Seven places where Fin's forecast produces a number the owner did not intend and nothing says so.
...
questions is irritating, while 500,000 of debt the model cannot see is wrong.
...
## 2. The seven, ranked by money

| # | What | Measured on prod 2026-08-05 | Detection |
|---|---|---|---|
| **R3** | **`House Morgage` is a loan the engine does not treat as a loan.** Typed `Loan`, 500,000 principal, 19 amortization rows, secured against the house, `loan_interest_rate` **NULL in all five scenarios**. The engine's loan branch is `loanRate != null`, so it books **no debt and no interest**. And `computeLoanWarnings` bails on the *same* condition — **the one guard that would catch this is switched off by the very NULL that causes it.** | 500,000 of debt absent from the model, ×5 scenarios | *"Typed Loan and carrying loan assumptions, but no interest rate — the engine is not treating this as a loan."* The general form is **type says X, data says not-X**, and it is what makes CR070's Tier B tolerable. |
...
3. **`House Morgage`** — is it supposed to carry an interest rate? *(Also asked in CR070 Q1, because
   it is the clearest example of that CR's Tier-B risk.)*
