...
⚠️ **`OCR_LLM_CLIENT_KEY` was printed in cleartext on 2026-09-06 and is pending an owner decision
on rotation.** An agent session ran `docker compose -f docker-compose.v4.yml config` to confirm the
...
was not sent to any third party, not written to a file, and not committed. **The documented trigger
for this var is "exposure / gateway re-keying", so this qualifies** — rotation is a two-sided
operation (ocr-llm holds the counterpart; all three Fin callers 401 the moment they disagree) and
is the owner's call, not an agent's. **The durable lesson is about the redaction, not the key:**
