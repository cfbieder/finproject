...
**Last updated:** 2026-07-12 · **Live version:** v3.0.76 (see `VERSION` / git tags)

## Current phase
- [CR045 — Forecast Cash Warnings & Forced Liquidation](../cr/cr-045-forecast-cash-warnings-liquidation.md): **Phase 1 (cash-health warnings pane on `/forecast-review`) + Phase 1b (BaseYear sweep seed) done — not yet deployed.** Two bugs, both silent-wrong-number: `copyScenario` never copied `cash_sweep_priority`, so every copied scenario ran with **no sweep module** and left shortfalls unfunded (prod "2026 with House Purchase": 14 shortfall years / −$20.1M → 3 / −$3.35M once re-ranked + regenerated); and the sweep never applied the **BaseYear cash flow** to its opening balance, holding the band ~a year of NCF above the bank line the Review displays. **Phase 1b deliberately breaks engine byte-parity** — prod must be regenerated after the deploy. Phase 2 (whole-asset liquidation + capital-gains tax on forced sweep sales) not started; §6 Q1 open.
