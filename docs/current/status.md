...
**Last updated:** 2026-09-05 · **Live version:** **v3.54.0** (see `VERSION` / git tags) — **v3.54.0: the bridge becomes a report, and the portfolio gets a decade of prices.** Two threads. **[CR092](../cr/cr-092-net-worth-bridge.md) P2** turns the Home hero's *What changed?* breakdown into **`/net-worth-drivers`** under Reports & Graphs — the same exact decomposition over any period via the sibling report's `PeriodSelector`, with **every** account in a grid sortable by any driver column. Its rendering is **shared** with the modal rather than copied, gated by all 11 modal tests passing unchanged. 🔴 **Rendering it on a year-to-date window exposed a defect in P0's prose that P0's own window could never show**: the lead named the largest driver by ABSOLUTE value, so a **$96,705 fall** read *"almost all of it is one thing: money earned added $368,591"* — earning money does not cause a fall. A leading driver must now share the change's sign, and cancelling drivers get **no** leader; `buildSummary` is tested directly, because both rules turn on a driver MIX no particular database is guaranteed to hold. **[CR093](../cr/cr-093-portfolio-xray.md)** replaces fin's **2,023** closes with **426,614** from Tradier back to **2014-01-02** — fin held 44 trading days, and MACD 12/26/9 would have been almost entirely warm-up. **146 of 168** per-share securities resolved; the 22 that do not are genuinely unquotable, so quotability stays EARNED. ⚠️ **Two feeds, one column** — every shared date beyond 0.1% is printed before adoption, because across providers the disagreement reaches **1.5%**. ⚠️ **The class-share separator is a SLASH**: `BRK.B` and `BRK-B` return nothing, `BRK/B` returns both bars and sector — the dot was asserted before being tested, and the assertion was wrong. 🔴 **And the classification it exposed was CORRECTED in the same release**: `FLDR` ($533,543, the largest holding), `HYG`, `NVG` and `AGG` are bond funds fin held as **equity** — **$566,878, 14.6% of the portfolio** — moving the split **48/43 → 33% equity / 58% BOND** on dev and prod. ⚠️ **No page was showing the wrong number** (`asset_class` renders nowhere), but every X-ray slice would have inherited it, and **no vendor could settle it alone** — Tradier returns `null` for every fund, FinImpulse files closed-end funds as `quote_type=stock` with no category, which is exactly where `NVG` sits. [The 28-fund sheet](../cr/cr-093-fund-classification-sheet.md) stays open for the rest
