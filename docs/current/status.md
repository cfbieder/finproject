...
- ⚠️ **The forecast needs one regenerate, held deliberately.** [CR064 P8](../cr/cr-064-forecast-annual-close-and-assumptions.md) (v3.11.10) corrected the base-year column on read — `2026 Base` now shows **−254,728** where it showed **+144,395** — but the stored `forecast_entries` were built on the old, FX-inflated opening cash, so the projected years and the base year currently disagree. A regenerate moves the sweep by **+1.26M** of extra sales across the horizon (no new shortfalls). It is **deferred on purpose**: United Beverages' `income_amount = 500,000` is unresolved (PLN as stored ⇒ ~128K USD; if USD was meant it should be 1,902,439 PLN), and rebuilding now would bake in a number that may be 4× out. **Settle UB, then regenerate once.**
...
- **Gates:** 770 backend / 319 frontend / 8 e2e tests; lint **blocking** (0 errors), plus six ratchets that may only shrink (lint-debt, api-envelope, buttons, modals, hex, tokens).
