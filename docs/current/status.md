...
**Last updated:** 2026-09-08 · **Live version:** **v3.60.0** (see `VERSION` / git tags) — **v3.60.0: what CASH pays**, closing the gap [CR093](../cr/cr-093-portfolio-xray.md) P3 shipped and named. Migration **080**. ⚠️ **Never a vendor question** — the custodian prints `-- 7-day yield: 3.29%` and `-- Interest rate: 1.82%` on every statement, and the row scanner **had to step over that clause to reach the numbers** and threw it away. ⚠️ **Two kinds kept apart** (an annualised fund yield is not a stated bank rate), ⚠️ **neither contractual** so both land in ESTIMATED, and ⚠️ **each is as of a statement** — this corpus runs 0.06% to 5.30%, so every row shows its date. **Income $112,297 → $116,037 (2.90% → 3.00%)**; cash stating nothing falls from $193,138 to **$76,573**. 🔴 **The estimate was wrong before it was measured**: §4a guessed ~$7,000/yr, the real figure is **$3,710** — most of the balance has no rate on any statement and the sweeps pay 1.82%, not 3.5%. ✅ **Also closed: [#26](project-roadmap.md#3-known-issues)** — `exposure.test.js` was green in CI and red on real data; `buildExposure` took the same fetch/pure split and **all 1,409 backend tests now pass on dev's real database as well as a fresh one**. ⚠️ The issue's recorded remedy would not have worked — the tests already built fixtures, and a portfolio builder reads every account, so a fixture is added to production data rather than replacing it. **And `FMP_API_KEY` is removed** — never verified, never wired to anything. 🔴 **Found while loading it: the feed RENAMED two FDIC sweeps to numeric ids on consecutive days and FORKED the securities** (`QIBZQ`→`FDIC91125` 2026-08-09, `QHYEQ`→`FDIC91075` 2026-09-04) — every snapshot still reconciles, which is why it went unnoticed, but the rate is stranded on the retired row and **$83,270 states no income where it should state ~$1,515/yr** ([#29](project-roadmap.md#3-known-issues), owner decision — merging two securities is an identity judgement).
