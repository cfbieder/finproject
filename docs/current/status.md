...
**Last updated:** 2026-08-01 · **Live version:** v3.11.1 (see `VERSION` / git tags)
...
- **[CR059](../cr/cr-059-fintable-api-ingestion.md) — fintable's REST API replaces the Google-Sheet scrape.** P0–P2 built and gate-verified; **nothing live** (`FINTABLE_SOURCE=sheets`). Remaining: P3a (31 account mappings, migration **044**) then P4 cutover. Retires the display-name join behind the Black Card incident.
...
- **Recent releases:** v3.11.1 (the period filter below) · v3.11.0 ([CR050 §10](../cr/cr-050-forecast-scenario-variants.md)) · v3.10.0 ([CR063](../cr/cr-063-coa-ordering.md), migration 049) · v3.8.0–v3.9.2 ([CR062](../cr/cr-062-forecast-loan-module.md) loans + equity, migrations 047/048).
...
- **CI had been red since 2026-07-30 — 30 pushes, three releases and a prod deploy over a failing gate (2026-08-01).** Two causes, the second hiding the first: four DB-backed suites borrowed accounts and an `fc_line` that only the *dev* database holds (29 tests dead in `beforeAll`), then migration **046** asserted a production data fact unconditionally and aborted the whole chain on CI's empty database, killing `backend-tests` and `e2e` before the tests ran. Suites now seed their own fixtures, `ci-seed.sql` carries `Interest Income`, and 046 skips when there is no mapping to guard. Green from scratch: 714 backend, 8 e2e. **Still open — nothing announces a red `main`:** see [Known Issue #12](project-roadmap.md#3-known-issues).
...
- Everything else: [roadmap §3](project-roadmap.md#3-known-issues) — 11 entries, including the timezone rule (#3), the 13 untriaged same-signed transfer clusters (#8), and the ESLint JSX blind spot (#10).
