...
**Last updated:** 2026-09-06 · **Live version:** **v3.59.1** (see `VERSION` / git tags) — **v3.59.1: both LLM-gateway callers were aborting BELOW ocr-llm's deadline** ([§3 #28](project-roadmap.md#3-known-issues)). Each bound must be looser than the one it wraps (`chain < their deadline_ms < our abort`), because their abort does not propagate on client disconnect — an abort below their deadline turns a typed 504 into a bare transport error and leaves an abandoned call pinning a shared GPU tier. 🔴 **`extract-statements-llm.js` (480→720 s) had already fired**: three `finance_statement_extract` answers above our old abort were computed and delivered to a **closed socket** — statements that silently did not extract, evidence only in a log in the other repo. **A gateway 200 does not mean the client received it.** 🟢 `aiReview.js` (300→660 s) was latent — 14 calls in four months, max 147.9 s — but its `ollama_mid` fallback was unreachable at 300 s, and *a fallback you stop listening for is not a fallback*. Both found in **one command**, the day ocr-llm exposed `deadline_ms`/`deadline_source`. Detail: [roadmap §1.2](project-roadmap.md).
