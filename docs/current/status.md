...
**Last updated:** 2026-07-12 · **Live version:** v3.0.87 (see `VERSION` / git tags)

## Current phase
- **v3.0.87 fix — module Type could not be set on an unmatched module.** The Type dropdown rendered the configurable **module-type** list (Real Estate, Stocks, Private Equity, …) only when the module was *matched* to a COA account; an unmatched one fell through to a generic branch offering the COA's **account** types (`asset`/`expense`/`income`/`liability`) instead, so a new property could not be typed "Real Estate" at all. Both prod's unmatched property modules ("New House", "Sarasota House") carry a lowercase `asset` module_type from that path — owner to re-type them. `module_type` is descriptive (the engine never reads it), so this is UI-only, no regenerate.
- [CR047 — Income-Only Tax Rate Override](../cr/cr-047-module-income-tax-override.md): **shipped v3.0.84 (migration 038).** `tax_rate_override` moved a module's capital-gains AND income rate together, so "the United Beverages dividend arrives already taxed in Poland — only ~3% incremental US tax — but a future *sale* is still an ordinary capital gain" could not be expressed. New `income_tax_rate_override` applies to income only (amount- and yield-based, incl. the deferred base-year income tax); gains keep `tax_rate_override`/the scenario rate, as does the CR045 sweep liquidation tax. NULL falls back ⇒ existing modules byte-identical; **0 is a real rate**, not "unset". Opt-in only. v3.0.85 renames the two fields to say what they tax: **Full Tax Override** (gains + income) and **Recurring Income Tax Override** (income only). **v3.0.86 fixes a silent save bug**: `FCModuleManage`'s PUT body is an explicit whitelist, and neither the tax override nor CR046's four window dates were in it — they were dropped on save with no error. Both CRs had been verified via `curl`, which bypassed the broken layer. The payload builder is now extracted and tested against `FIELD_SECTIONS`, so no editor field can be silently dropped again.
