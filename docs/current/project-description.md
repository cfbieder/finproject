...
├── server/                         # Express API
│   ├── db/migrations/              # 001..036 SQL — registry in docs/current/migrations.md
...
311 backend Jest tests (engine incl. CR041 ownership gating + CR043 generate transactionality, the CR043 migration runner, **route-contract tests for `forecast.js` + `budget.js` + `reports.js`** (CR043 Phase 1.2 / 2.2), services incl. aiReview compare, repositories, scripts, `v2/utils/validate` + `AppError`; DB-backed suites self-seed against `DATABASE_URL`) — run `cd server && npm test`. 129 frontend Vitest tests (utils helpers + `fcCompareUtils` diff engine + `FIELD_SECTIONS` grouping + `rest.js` timeout + `useCoa`/`useOverview` hooks via `@testing-library/react`) — `cd frontend && npm test`. HTTP smoke: `node server/src/scripts/smoke-after-021.js`. **CI** (`.github/workflows/ci.yml`) runs the backend suite against a fresh migrations+[`ci-seed.sql`](../../server/db/ci-seed.sql) Postgres, the **frontend Vitest suite (blocking, CR043 Phase 1.1)** + build (lint still advisory until the 160-error debt clears), and a tracked-secret grep gate. Inventory: [test-overview.md](test-overview.md).
