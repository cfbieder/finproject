...
| `check-ci.sh` | What does CI say about a commit? (`[ref] [--quiet] [--wait[=N]]`). Exit code is the interface — **0** green · **1** red · **2** pending · **3** no run · **4** could not ask GitHub. `--wait` blocks while the verdict can still change (run in flight, or a pushed commit GitHub has not registered), never on a settled or unpushed one. Answers [Known Issue #12](project-roadmap.md#3-known-issues) at session start |
