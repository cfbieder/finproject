...
const { summariseExposure, summariseFixedIncome, gradeOf } = require('../exposure');

const dbDescribe = process.env.SKIP_DB_TESTS ? describe.skip : describe;

/**
 * 🔴 NO DATABASE — this block is roadmap issue #26, closed.
 *
 * These seven were DB-backed and asserted fixture-scale figures against a
 * builder that reads EVERY account's latest snapshot. They were green on CI's
 * empty database and red on dev's real one (`expected 600, received
 * 456,344.58`), and every gate built to catch a red suite reported green —
 * precisely BECAUSE `ci-seed.sql` has no positions, so the assertions never met
 * data.
 *
 * ⚠️ The issue's own suggested fix — "build the fixture" — would NOT have
 * worked: they already seeded throwaway securities and positions, and the
 * fixture is ADDED to production-scale data rather than replacing it. The
 * builder is now a thin fetch plus this pure function, so the rules are pinned
 * by rows that exist only here.
 */
describe('CR093 summariseExposure — the coverage rules, on invented rows', () => {
  const sec = (over = {}) => ({
    id: over.id ?? 1,
    ticker: over.ticker || 'ZZ',
    name: over.ticker || 'ZZ',
    asset_class: over.asset_class || 'equity',
    price_basis: over.price_basis || 'per_share',
    fund_category: null,
    asked: over.asked || false,
    mv: over.mv,
  });
  const w = (security_id, sector, weight) => ({ security_id, sector, weight });
  const find = (rows, key, val) => rows.find((r) => r[key] === val);

  test('a fund is spread across its sectors by weight, not booked whole', () => {
    const r = summariseExposure([sec({ id: 1, ticker: 'ZZFUND', mv: 1000 })],
      [w(1, 'technology', 0.6), w(1, 'healthcare', 0.4)]);
  ...
  test('🔴 a bond is NOT-APPLICABLE by nature, and never a gap to be closed', () => {
    // One "unclassified" bucket for both absences would leave the owner unable
    // to tell a bond-heavy portfolio from a broken pipeline.
    const r = summariseExposure([sec({ ticker: 'ZZBOND', asset_class: 'bond', mv: 5000 })], []);
  ...
  test('🔴 a par instrument is not-applicable even when its asset_class is unknown', () => {
    // Three FDIC deposits are classed `unknown` and held at par. Reading them as
    // "not covered" would park cash in a bucket meant to shrink.
    const r = summariseExposure(
      [sec({ ticker: 'ZZFDIC', asset_class: 'unknown', price_basis: 'par', mv: 7000 })], []);
    expect(r.sector_coverage.not_covered).toHaveLength(0);
    expect(Number(r.sector_coverage.not_applicable_value)).toBeCloseTo(7000, 2);
  });

  test('🔴 an EQUITY holding with no weights and no as_of IS not-covered', () => {
    // The closed-end funds. Both vendors report them as financial_services —
    // their manager's sector — so the loader refuses and leaves no as_of. They
    // must surface, not vanish into "not applicable".
    const r = summariseExposure([sec({ ticker: 'ZZCEF', mv: 900 })], []);
  ...
  test('an equity ASKED about, with no sectors, is not-applicable rather than not-covered', () => {
    const r = summariseExposure([sec({ ticker: 'ZZASKED', asked: true, mv: 400 })], []);
    expect(r.sector_coverage.not_covered).toHaveLength(0);
    expect(Number(r.sector_coverage.not_applicable_value)).toBeCloseTo(400, 2);
  });

  test('🔴 nothing uncovered is redistributed across the sectors we DO know', () => {
    // Spreading it pro-rata invents exposure never measured, and is the shape
    // that makes a chart look complete while describing the wrong total.
    const r = summariseExposure(
      [sec({ id: 1, ticker: 'ZZF2', mv: 1000 }), sec({ id: 2, ticker: 'ZZG2', mv: 1000 })],
      [w(1, 'energy', 1.0)]);
  ...
  test('sector shares carry BOTH denominators', () => {
    ...
    // look like 32%.
    const r = summariseExposure(
      [sec({ id: 1, ticker: 'ZZF3', mv: 1000 }), sec({ id: 2, ticker: 'ZZB3', asset_class: 'bond', mv: 3000 })],
      [w(1, 'technology', 1.0)]);
  ...
  test('an empty portfolio does not divide by zero', () => {
    const r = summariseExposure([], []);
    expect(r.by_sector).toEqual([]);
    expect(Number(r.total_market_value)).toBe(0);
    expect(r.sector_coverage.share_sectored).toBe(0);
  });
});
