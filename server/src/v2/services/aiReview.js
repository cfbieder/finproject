...
UNITS — read this before proposing any growth number.
- growth_rate is a MULTIPLIER OF INFLATION, not a percentage. 1 means the asset grows exactly with
  inflation; 0 means it never grows in nominal terms; 2 means twice inflation. At 2.5% inflation,
  growth_rate 1 = 2.5%/yr and growth_rate 4 = 10%/yr. Typical values are between 0 and 2 — a value
  above 2 is a very aggressive assumption, so say so explicitly if you propose one.
- tax_rate_override IS a percentage (19 means 19%).
Proposing a percentage where a multiplier is expected silently multiplies the plan's growth several
times over, and the resulting figure looks entirely ordinary.
...
async function buildForecastContext(scenarioName) {
  ...
  // The scenario's first inflation rate, so a module's growth MULTIPLIER can be printed as the
  // effective rate it actually produces rather than left for the reader to infer.
  const inflationPct = (config?.inflationRates || [])[0];
  ...
  for (const mod of modules) {
    ...
    // ⚠️ `growth_rate` is a MULTIPLIER of inflation, never a percent — and printing it as
    // `1% (×inflation)` invited exactly the misreading it was meant to prevent. Observed on prod
    // 2026-08-14: the model proposed `growth_rate → 4` for `Fidelity Stocks` (then at 1.0000),
    // reasoning "a more realistic nominal growth rate for US equities" — i.e. it read 4 as 4%,
    // when 4 means 4 × inflation ≈ 10%/yr. The unit is now stated without a percent sign, and the
    // effective rate is spelled out so the model never has to infer it.
    const infl = Number(inflationPct);
    const mult = Number(mod.growth_rate);
    const effective = Number.isFinite(infl) && Number.isFinite(mult)
      ? ` = ${(mult * infl).toFixed(2)}%/yr at ${infl}% inflation`
      : "";
    lines.push(`  Growth: ${mod.growth_rate}× inflation${effective}, Gains Tax Override: ${mod.tax_rate_override ?? 'default'}`);
