...
Adapted from starter pack v1.8.3 (`docs/starter-pack/.claude/skills/brief/`). Fix practice there first, then sync this copy.
...
- **CR progress** — count the index **rows**, never its hand-kept *Summary by status* table:
  ...
  ```
  If it disagrees with the roll-up table, show the derived figure and add a bucket-A item to fix the table.
- **Known issues** — `awk '/^## 3\. Known Issues/{f=1;next} /^## 4\./{f=0} f' docs/current/project-roadmap.md | grep -oE '^- \[[ x]\]' | sort | uniq -c`.
- **Open items** — `docs/current/status.md` → *Next* and `project-roadmap.md` §3's unchecked
  boxes, each cross-checked against its CR's own status line (drop anything already done), then
  bucketed by blocker:
  - **A** — nothing named is in the way. Most CR increments land here.
  - **B** — items under status.md's *"With the owner, do not start unasked"*, every forecast
    judgement, and any CR whose own file records an unresolved owner decision. A DRAFT CR
    awaiting review is **B**: the decision is go / revise / defer, not code.
  - **C** — name the blocker AND its distance, measured this build: an ocr-llm handoff (name
    the thread and its age from `.claude/hooks/handoff-inbox.sh`), a gateway task confirmed
    absent from the live contract, a dated cutoff, a count that has to grow before a measurement
    separates. ⚠️ A blocked feed item is only C while the endpoint still says so — re-run the
    check; `attention-summary` reading 0 moves it off the page, not into C.
